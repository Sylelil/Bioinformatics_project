...
def save_numpy_features(dir, slidename, path_to_save):
    ...
    level = levels[1]
    save_path = os.path.join(str(path_to_save), slidename.split('.')[0] + str(level) + '.npy')
    ...
    np.save(os.path.join(str(path_to_save), slidename.split('.')[0] + str(level) + '.npy'), X)
...
def main():
    ...
    images_folder = BASE_DIR / Path('datasets') / 'images'
    classes = ('normal', 'tumor')

    normal_images_dir = os.path.join(str(images_folder), classes[0])
    tumor_images_dir = os.path.join(str(images_folder), classes[1])

    normal_images_list = get_all_slides(normal_images_dir)
    tumor_images_list = get_all_slides(tumor_images_dir)

    normal_images_save_dir = BASE_DIR / Path('extracted_features') / 'images' / 'numpy_normal'
    tumor_images_save_dir = BASE_DIR / Path('extracted_features') / 'images' / 'numpy_tumor'

    if not os.path.exists(str(BASE_DIR / Path('extracted_features')) ):
        os.mkdir('extracted_features')
    if not os.path.exists(str(normal_images_save_dir)):
        os.mkdir(str(normal_images_save_dir))
    if not os.path.exists(str(tumor_images_save_dir)):
        os.mkdir(str(tumor_images_save_dir))
