...
def main():
    ...
    if num_principal_components is not None:
        ...
        path_to_save_pca = Path(paths.concatenated_results_dir) / f"pca_{num_principal_components}"
    ...
    if gene_copy_ratio is not None:
        ...
        path_to_save_copied_genes = paths.concatenated_results_dir / f'copy_ratio_{gene_copy_ratio}'
        if not os.path.exists(path_to_save_copied_genes):
            os.makedirs(path_to_save_copied_genes)

        if gene_copy_ratio <= 0:
            print(f'error: invalid argument <gene_copy_ratio>: {gene_copy_ratio}')
